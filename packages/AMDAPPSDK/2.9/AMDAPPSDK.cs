...
namespace AMDAPPSDK
{
    ...
    class AMDAPPSDK : C.ThirdPartyModule
    {
        static AMDAPPSDK()
        {
            ...
            if (null == InstallPath)
            {
                using (var key = Opus.Core.Win32RegistryUtilities.Open32BitLMSoftwareKey(@"AMD\APPSDK\2.9"))
                {
                    if (null == key)
                    {
                    ...
                    Opus.Core.Log.DebugMessage("AMD APP SDK 2.9: Installation path from registry '{0}'", installPath);
