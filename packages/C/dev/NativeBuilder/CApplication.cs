        ...
        public object Build(C.Application application, Opus.Core.DependencyNode node, out bool success)
        {
            ...
            if (node.Children != null)
            {
                foreach (Opus.Core.DependencyNode node1 in node.Children)
                {
                    ...
                    else if (node1.Module is C.DynamicLibrary)
                    {
                        C.LinkerOptionCollection options = node1.Module.Options as C.LinkerOptionCollection;
                        linkerOptions.Libraries.AddToFront(options.StaticImportLibraryFilePath);
                    }
