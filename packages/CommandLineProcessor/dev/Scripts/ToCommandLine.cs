        ...
        public static void Execute(object sender, Opus.Core.StringArray commandLineBuilder, Opus.Core.Target target)
        {
            ...
            foreach (System.Collections.Generic.KeyValuePair<string, Opus.Core.Option> optionKeyValue in optionCollection)
            {
                ...
                if (null != option.PrivateData)
                {
                    ...
                    if (null == data)
                    {
                        throw new Opus.Core.Exception(System.String.Format("Option data for '{0}', of type '{1}', does not implement the interface '{2}'", optionName, option.PrivateData.GetType().ToString(), typeof(ICommandLineDelegate).ToString()));
                    }
                ...
                {
                    throw new Opus.Core.Exception(System.String.Format("Option '{0}' is not set up for command line translation", optionName), false);
                }
            }
        }
    }
