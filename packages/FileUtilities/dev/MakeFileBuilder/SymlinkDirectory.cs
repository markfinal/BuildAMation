...
namespace MakeFileBuilder
{
    public sealed partial class MakeFileBuilder
    {
        public object Build(FileUtilities.SymlinkDirectory moduleToBuild, out bool success)
        {
            ...
            {
                recipe += System.String.Format(" {0} /D $@ $<", commandLineBuilder.ToString(' '));
            }
            else
            {
            ...
            var sourceDirectoryPath = moduleToBuild.SourceFileLocation.GetSinglePath();
#if true
            var rule = new MakeFileRule(
                moduleToBuild,
                FileUtilities.SymlinkFile.OutputFile,
                node.UniqueModuleName,
                dirsToCreate,
                null,
                new Opus.Core.StringArray(sourceDirectoryPath),
                recipes);
#else
