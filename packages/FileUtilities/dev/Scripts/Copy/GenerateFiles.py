...
# FileUtilities copyfile options
copyfile_options = [
    opusCodeGeneratorExe,
    "-i=" + os.path.relpath(os.path.join(opusPackageDir, "FileUtilities", "dev", "Scripts", "Copy", "ICopyFileOptions.cs")),
    "-n=FileUtilities",
    "-c=CopyFileOptionCollection",
    "-p", # generate properties
    "-d", # generate delegates
    "-dd=" + os.path.relpath(os.path.join(opusPackageDir, "CommandLineProcessor", "dev", "Scripts", "CommandLineDelegate.cs")),
    "-pv=PrivateData"
]
copyfile_options.extend(extra_args)
(stdout,stderr) = ExecuteProcess(copyfile_options, True, True)
print stdout
