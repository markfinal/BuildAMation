...
namespace Qt
{
    public sealed class Toolset : QtCommon.Toolset
    {
        ...
        protected override string GetInstallPath(Bam.Core.BaseTarget baseTarget)
        {
            ...
            {
                using (Microsoft.Win32.RegistryKey key = Bam.Core.Win32RegistryUtilities.OpenCUSoftwareKey(@"Microsoft\Windows\CurrentVersion\Uninstall\Qt 5.1.1"))
                {
                    if (null == key)
                    {
                        throw new Bam.Core.Exception("Qt libraries for 5.1.1 were not installed");
                    }

                    installPath = key.GetValue("InstallLocation") as string;
                    if (null == installPath)
                    {
                        throw new Bam.Core.Exception("Unable to locate InstallLocation registry key for Qt 5.1.1");
                    }
