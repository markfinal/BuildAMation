...
namespace QtCommon
{
    ...
    {
        public Declarative(System.Type toolsetType, bool includeModule)
        {
        ...
        {
            options.OutputPaths[C.OutputFileFlags.Executable] = new Opus.Core.StringArray(this.GetModuleDynamicLibrary(this.ToolsetType, target, "QtDeclarative"));
        ...
        void QtDeclarative_IncludePaths(Opus.Core.IModule module, Opus.Core.Target target)
        {
            ...
            if (null != options)
            {
                this.AddIncludePath(this.ToolsetType, options, target, "QtDeclarative", this.IncludeModule);
            }
        }
    }
}
